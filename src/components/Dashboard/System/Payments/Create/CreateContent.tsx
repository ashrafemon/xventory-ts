...
import { createPaymentMethod } from "../../../../../store/actions/dashboard/system/paymentActions";
    ...
    const submitHandler = (e) => {
        ...
        if(!nameValidate && !codeValidate){
            ...
            Object.keys(form).forEach((key) => {
                if (key !== "storeList") {
                    paymentData[key] = form[key];
                } else {
                    form.storeList.forEach((item) => {
                        paymentData[key].push({ id: item.id });
                    });
                }
            });

            dispatch(createPaymentMethod(paymentData, resetForm));
        }
    };
